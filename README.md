...
- 선착순 쿠폰 지급
    ...
    - 해결
        - 선착순 n명에게 쿠폰을 정확히 지급하기 위해 RDS Lock 과 Redis 고려
            - 위 재고는 재고에만 lock 걸어주면 되므로 (재고 감소 로직) pessimistic lock 으로 락 구현
            - 하지만 쿠폰 로직에서 걸어야하는 락 구간이 길기 때문에 성능 불이익이 발생
            - 인 메모리인 Redis 로 락 구현하기로 최종 결정
        - Redis 의 Redisson 으로 락 구현
        - Redis 의 incr 으로 성능 개선
            - **`"coupon_count"`** 키에 저장된 숫자 값을 원자적으로 증가시켜서 인 메모리로 카운트 (발급받은 쿠폰 수) 를 추적함
            - RDB 에서 추적한 소요 시간 단축
